  ...
  dir("${env.WORKSPACE}/folio-integration-tests") {
    withMaven(
      ...
      withCredentials([usernamePassword(credentialsId: 'testrail-ut56', passwordVariable: 'testrail_password', usernameVariable: 'testrail_user')]) {
        sh "mvn clean test -Dkarate.env=${okapiDns} -pl quick-marc -DfailIfNoTests=false -Dtestrail_url=${TestRailUrl} -Dtestrail_userId=${testrail_user} -Dtestrail_pwd=${testrail_password} -Dtestrail_projectId=${TestRailProjectId}"
      }
    }
