...
def runIntegrationTests(ctx) {
  ...
  dir("${env.WORKSPACE}/folio-integration-tests") {
    withMaven(
      ...
      sh "mvn clean test -Dtest=QuickMarcApiTest -pl quick-marc  -DargLine="-Dkarate.env=${okapiDns}" -DfailIfNoTests=false -Dtestrail_url=${TestRailUrl} -Dtestrail_userId=${testrail_user} -Dtestrail_pwd=${testrail_password} -Dtestrail_projectId=${TestRailProjectId}"
      }
    }
