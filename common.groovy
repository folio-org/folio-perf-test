...
def main() {
  ...
  def awsKeyId = "${params.JenkinsAwsCredential}"
  def sshKeyId = "${params.JenkinsEc2Credential}"
  ...
  withCredentials([[$class: 'AmazonWebServicesCredentialsBinding', credentialsId: "${awsKeyId}",
    ...
    stage("Create Environment") {
      ...
      cmd += " ParameterKey=PublicIp,ParameterValue=${params.AwsUsePublicIp}"
    ...
    sshagent(credentials:["${sshKeyId}"]) {
      stage("Wait for Environment") {
        ...
        waitUntil {
          try {
            sh "ssh -o StrictHostKeyChecking=no -l ${sshUser} ${dbIp} docker ps -l"
            sh "ssh -o StrictHostKeyChecking=no -l ${sshUser} ${modsIp} docker ps -l"
            sh "ssh -o StrictHostKeyChecking=no -l ${sshUser} ${okapiIp} docker ps -l"
            true
          } catch (e) {
            sleep 10
            false
      ...
      stage("Bootstrap DB") {
        ...
        timeout(10) {
          waitUntil {
            try {
              def rs = sh(script: "ssh -o StrictHostKeyChecking=no -l ${sshUser} ${dbIp} 'docker ps | grep foliodb | wc -l'", returnStdout: true)
              rs.toInteger() >= 1
            } catch (e) {
              sleep 10
              false
      ...
      stage("Bootstrap Okapi") {
        ...
        sh "ssh -o StrictHostKeyChecking=no -l ${sshUser} ${okapiIp} \"${okapiJob}\""
        timeout(5) {
          waitUntil {
            try {
      ...
      stage("Bootstrap modules") {
        ...
        sh "scp -pr -o StrictHostKeyChecking=no folio-conf ${sshUser}@${modsIp}:/tmp"
        sh "scp -pr -o StrictHostKeyChecking=no folio-conf ${sshUser}@${dbIp}:/tmp"
        ...
        deployMods(mods, okapiIp, modsIp, modsPvtIp, tenant, sshUser)
      }
      ...
      stage("Populate data") {
        ...
        sh "ssh -o StrictHostKeyChecking=no -l ${sshUser} ${dbIp} tar -zxvf ${dataName}.tar.gz"
        sh "ssh -o StrictHostKeyChecking=no -l ${sshUser} ${dbIp} \"cd ${dataName} && ${cmd}\""
      }
    }
...
def deployMods(mods, okapiIp, modsIp, modsPvtIp, tenant, sshUser) {
  ...
  for (entry in mods.entrySet()) {
    ...
    sh "ssh -o StrictHostKeyChecking=no -l ${sshUser} ${modsIp} ${modJob}"
